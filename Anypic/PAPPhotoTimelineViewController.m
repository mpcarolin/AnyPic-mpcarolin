...
// this is the most significant, and complex, method in this View. It handles querying Parse
// for photographs to populate the timeline. Other methods focus merely on view / ui elements
- (PFQuery *)queryForTable {
    ...
    // create a query object -- but do not execute yet -- for the tableView to execute
    // and query parse's database LATER. This query object will find all the photos
    // uploaded by friends whom the current user follows
    PFQuery *followingActivitiesQuery = [PFQuery queryWithClassName:kPAPActivityClassKey];
    [followingActivitiesQuery whereKey:kPAPActivityTypeKey equalTo:kPAPActivityTypeFollow];     // filter activities to "follow"
    [followingActivitiesQuery whereKey:kPAPActivityFromUserKey equalTo:[PFUser currentUser]];   // further filter for current user's "follow" activities
    ...
    // create a query object for user's own uploaded photos
    PFQuery *photosFromCurrentUserQuery = [PFQuery queryWithClassName:self.parseClassName]; // all PObjects have a parseClassName var
    [photosFromCurrentUserQuery whereKey:kPAPPhotoUserKey equalTo:[PFUser currentUser]];    // class method returning current logged-in user
    [photosFromCurrentUserQuery whereKeyExists:kPAPPhotoPictureKey];

    // combine the following and current user queries into one to construct the timeline
...
// The beginning of this pipeline was the PAPPhotoHeaderView. It registered a "like", which directed to here, its delegate
// class. Now, because we are liking the photo, we use this delegate method.
- (void)photoHeaderView:(PAPPhotoHeaderView *)photoHeaderView didTapLikePhotoButton:(UIButton *)button photo:(PFObject *)photo {
    // disable the button so users cannot send duplicate requests to like the same photo
    [photoHeaderView shouldEnableLikeButton:NO];
    
    // set the new state of the button (to dark or to light)
    BOOL liked = !button.selected;
    [photoHeaderView setLikeStatus:liked];
    
    // gather the current number of likes on the photo
    NSString *originalButtonTitle = button.titleLabel.text;
    
    NSNumberFormatter *numberFormatter = [[NSNumberFormatter alloc] init];
    [numberFormatter setLocale:[[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]];
    
    // update papcache like count
    NSNumber *likeCount = [numberFormatter numberFromString:button.titleLabel.text];
    // increment cache if user liked the photo
    if (liked) {
        likeCount = [NSNumber numberWithInt:[likeCount intValue] + 1];
        [[PAPCache sharedCache] incrementLikerCountForPhoto:photo];
    // decrement cache if user un-liked a previously liked photo
    } else {
    ...
    // add current user as a liker (or not) of the photo in PAPCache
    ...
    // call appropriate static method to handle creating/deleting the right object
    if (liked) {
        [PAPUtility likePhotoInBackground:photo block:^(BOOL succeeded, NSError *error) {
            ...
            if (!succeeded) {
                // revert the like-count on the button to the original if call failed
